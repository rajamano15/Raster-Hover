...
export default function DemoForm({
  ...
  const [values, setValues] = useState<Record<FieldName, string>>({
    ...
    country: "",
    solution:
      presetSolution && DEMO_SOLUTIONS.includes(presetSolution as never)
        ? presetSolution
        : "",
    message: "",
  });
  ...
  const validate = (): FieldErrors<FieldName> => {
    ...
    if (!values.company.trim())
      e.company = "Please enter your company or hospital.";
    ...
    if (!values.solution) e.solution = "Please select a solution.";
    return e;
  };
  ...
  const onSubmit = async (ev: FormEvent) => {
    ...
    await submitEnquiry({ kind: "demo", ...values });
    setStatus("success");
  };
      ...
      >
        ...
        <div className="max-w-sm space-y-2">
          ...
          <p className="text-[0.92rem] leading-relaxed text-mist">
            ...
            contact you at {values.email} to arrange your demonstration
            {values.solution ? ` of ${values.solution}` : ""}.
          </p>
        </div>
      </motion.div>
    );
    ...
    <form ref={formRef} onSubmit={onSubmit} noValidate>
      <div className={gridCls}>
        ...
        <FieldWrap
          ...
          hint="Optional — tell us about your environment or requirements."
        >
          <TextArea
            id={`${idPrefix}-message`}
            name="message"
      ...
      <div className="mt-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <p className="text-[0.75rem] leading-relaxed text-faint">
          No pricing surprises — our team walks you through everything.
        </p>
      </div>
    </form>
  );
